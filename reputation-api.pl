...
Readonly my $EXPIRATION => 86400;
Readonly my @collections => split / /, $ENV{REPUTATION_API_COLLECTIONS};
###############################################################################

###############################################################################
## Helper for redis
helper redis => sub { 
    shift->stash->{redis} ||= Mojo::Redis2->new(url => $ENV{REPUTATION_API_REDIS_URL} );
};

###############################################################################
## Helper for mysql
my $dbh = DBI->connect( "DBI:mysqlPP:$ENV{REPUTATION_API_DB_NAME}:$ENV{REPUTATION_API_DB_HOST}",$ENV{REPUTATION_API_DB_USER},$ENV{REPUTATION_API_DB_PASS}) or exit;
helper db => sub { $dbh };
